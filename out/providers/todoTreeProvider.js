...
exports.TodoTreeProvider = void 0;
...
class TodoTreeProvider {
    ...
    constructor(context) {
        ...
        this.todoIconPath = vscode.Uri.joinPath(context.extensionUri, "resources", "todo.svg");
        this.loadTodos();
    }
    refresh() {
    ...
    updateCategories() {
        ...
        this.categories = Array.from(categoryMap.entries()).map(([category, todos]) => ({ category, todos }));
    }
    getTreeItem(element) {
    ...
    getSubTaskTreeItem(subtask) {
        const item = new vscode.TreeItem(subtask.text, vscode.TreeItemCollapsibleState.None);
        item.iconPath = new vscode.ThemeIcon(subtask.completed ? "check" : "circle-outline");
        item.description = subtask.completed ? "Completed" : "";
        item.contextValue = "subtask";
        return item;
    }
    getTagGroupTreeItem(tagGroup) {
        const item = new vscode.TreeItem(`#${tagGroup.tag} (${tagGroup.todos.length})`, vscode.TreeItemCollapsibleState.Expanded);
        item.iconPath = new vscode.ThemeIcon("tag");
        item.contextValue = "tagGroup";
        return item;
    }
    getCategoryGroupTreeItem(categoryGroup) {
        const item = new vscode.TreeItem(`${categoryGroup.category} (${categoryGroup.todos.length})`, vscode.TreeItemCollapsibleState.Expanded);
        item.iconPath = new vscode.ThemeIcon("folder");
        item.contextValue = "categoryGroup";
        return item;
    }
    getTodoTreeItem(todo) {
        const item = new vscode.TreeItem(todo.text, todo.subtasks.length > 0
            ? vscode.TreeItemCollapsibleState.Collapsed
            : vscode.TreeItemCollapsibleState.None);
        item.iconPath = new vscode.ThemeIcon(constants_1.TODO_ICONS[todo.type]);
    ...
    getTodoDescription(todo) {
        ...
        parts.push(constants_1.PRIORITY_ICONS[todo.priority]);
    ...
    getChildren(element) {
        if (!element) {
            return Promise.resolve(this.getSortedTodos().filter((todo) => this.filterTodo(todo)));
        }
        else if (this.isTagGroup(element)) {
            return Promise.resolve(element.todos.filter((todo) => this.filterTodo(todo)));
        }
        else if (this.isCategoryGroup(element)) {
            return Promise.resolve(element.todos.filter((todo) => this.filterTodo(todo)));
        }
    ...
    compareTodos(a, b) {
        switch (this.sortOption) {
            case "priority": {
                const priorityDiff = constants_1.PRIORITY_ORDER[a.priority] - constants_1.PRIORITY_ORDER[b.priority];
    ...
    async toggleTodo(todo) {
        try {
            ...
            const strikethroughText = todo.completed
                ...
                : lineText.replace(/(TODO:|FIXME:|BUG:|HACK:|NOTE:|OPTIMIZE:)(.+)/, "$1~~$2~~");
    ...
    loadTodos() {
        try {
            const todosData = this.context.globalState.get("todoify.todos", []);
            this.todos = todosData
                .map((todoData) => {
                try {
                    const todo = {
                        id: todoData.id || (0, utils_1.generateId)(),
                        text: todoData.text || "",
                        file: todoData.file || "",
                        ...
                        subtasks: (todoData.subtasks || []).map((st) => ({
                            id: st.id || (0, utils_1.generateId)(),
                            text: st.text || "",
    ...
    async startInlineEdit(todo, field) {
        try {
